...
function Length() {
  ...
  const options = lengthUnits.map((unit)=>unit.key)  // return the keys/name of different Lewngth. 
  // console.log(options);
  ...
  const convert = () => {
    if (amount < 0) {
      ...
      else {setConvertedAmount(convertLength(amount, from , to))}
    }
